...
from otm_core.dicts import origens, equipms_unpe, equipms_unbc, veiculos_unbc
from otm_core.calculations import DROP_INPUT_COLUMNS, DROP_INPUT_WITH_FRETE, CD_SUZANO_ORIGENS, format_date, direct_mult_split
from otm_core.multicoleta import suz_mult, multicoleta
...
def rate_offering_gid(model, unity, equipms):
  cd_suzano  = model['ORIGEM'].isin(CD_SUZANO_ORIGENS)
  veiculo_eq = model['VEICULO'].map(equipms).fillna('')
  sap_prefix = f'SUZANO.{unity}_0000' + model['SAP'].astype(str)

  model['EQUIPMENT_GROUP_PROFILE_GID'] = np.where(cd_suzano, veiculo_eq + '_CD_SUZANO', veiculo_eq)
  model['RATE_OFFERING_GID']           = np.where(cd_suzano, sap_prefix + '_CD_SUZANO', sap_prefix)
...
def build_model_rg(model, unity, mult_flag=False):
  ...
  df_rg      = _build_sql_header_frame(columns_rg, "EXEC SQL ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD'")
  ...
  if unity not in ['UNC', 'UNPE_CABOTAGEM']:
  ...
  rate_geo = pd.concat([df_rg, model.drop(columns=DROP_INPUT_WITH_FRETE)], ignore_index=True)
...
def build_model_rgcg(model, unity, mult_flag):
  ...
  df_rgcg      = _build_sql_header_frame(columns_rgcg, "EXEC SQL ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD'")
...
def df_rgcv_cols(model, unity):
  ...
  df_rgcv       = _build_sql_header_frame(columns_rgcv, "EXEC SQL ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD HH:MI:SS'")
  ...
  rate_geo_cost_viagem = pd.concat([df_rgcv, model.drop(columns=DROP_INPUT_COLUMNS)], ignore_index=True)
