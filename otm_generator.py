...
def rate_geo(model, unity, mult_flag=False, opl_flag=False):
  ...
  if unity == 'UNBC' and opl_flag:
    for col in ['RATE_GEO_GID', 'RATE_GEO_XID', 'RATE_OFFERING_GID']:
      if col in rate_geo.columns:
        rate_geo[col] = rate_geo[col].str.replace('UNBC_', 'UNBC_OPL_', regex=False)
    for col in ['RATE_GEO_COST_GROUP_GID', 'RATE_GEO_COST_GROUP_XID', 'RATE_GEO_GID']:
      if col in rate_geo_cost_group.columns:
        rate_geo_cost_group[col] = rate_geo_cost_group[col].str.replace('UNBC_', 'UNBC_OPL_', regex=False)
